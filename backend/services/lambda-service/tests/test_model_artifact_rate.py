...
class TestModelArtifactRate:
    ...
    @patch('app.jobs.model_artifact_rate.Rating_Model')
    @patch('app.jobs.model_artifact_rate.Artifact_Model')
    def test_lambda_handler_success(self, mock_artifact, mock_rating):
        ...
        # Mock rating instance
        mock_rating_instance = Mock()
        mock_rating_instance.name = 'test-model'
        mock_rating_instance.category = 'nlp'
        mock_rating_instance.net_score = {'value': 0.85, 'latency': 1.2}
        mock_rating_instance.ramp_up_time = {'value': 0.75, 'latency': 0.5}
        mock_rating_instance.bus_factor = {'value': 0.6, 'latency': 0.3}
        mock_rating_instance.performance_claims = {
            'value': 0.9, 'latency': 1.5
        }
        mock_rating_instance.license = {'value': 1.0, 'latency': 0.1}
        mock_rating_instance.dataset_and_code_score = {
            'value': 0.8, 'latency': 2.0
        }
        mock_rating_instance.dataset_quality = {
            'value': 0.7, 'latency': 1.0
        }
        mock_rating_instance.code_quality = {'value': 0.85, 'latency': 1.5}
        mock_rating_instance.reproducibility = {
            'value': 0.65, 'latency': 2.5
        }
        mock_rating_instance.reviewedness = {'value': 0.55, 'latency': 0.8}
        mock_rating_instance.tree_score = {'value': 0.7, 'latency': 1.2}
        mock_rating_instance.size_score = {
            'value': {
                'raspberry_pi': 0.3,
                'jetson_nano': 0.5,
                'desktop_pc': 0.8,
        ...
        mock_rating.find_by_artifact_id.return_value = mock_rating_instance
        ...
        assert result['net_score_latency'] == 1.2
        assert result['size_score']['raspberry_pi'] == 0.3
        assert result['size_score_latency'] == 0.5
    ...
    @patch('app.jobs.model_artifact_rate.Rating_Model')
    @patch('app.jobs.model_artifact_rate.Artifact_Model')
    def test_lambda_handler_not_model_artifact(
        self, mock_artifact, mock_rating
    ):
    ...
    @patch('app.jobs.model_artifact_rate.Rating_Model')
    @patch('app.jobs.model_artifact_rate.Artifact_Model')
    def test_lambda_handler_rating_not_found(
        self, mock_artifact, mock_rating
    ):
        ...
        # No rating found
        mock_rating.find_by_artifact_id.return_value = None
