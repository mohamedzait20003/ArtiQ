...
def check_license_compatibility(
    ...
    # 3. Restrictive artifact + Restrictive GitHub
    #    = Check if same license family
...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(
            f"[LICENSE_CHECK] Checking if artifact {artifact_id} exists"
        )
        artifact = Artifact_Model.get({'id': artifact_id})

        if not artifact:
            logger.warning(
                f"[LICENSE_CHECK] Artifact not found: {artifact_id}"
            )
            return (
        ...
        if not github_url.startswith('https://github.com/'):
            logger.warning(
                f"[LICENSE_CHECK] Invalid GitHub URL: {github_url}"
            )
            return (
                {
                    'errorMessage':
        ...
        try:
            ...
            parts = github_url.replace(
                'https://github.com/', ''
            ).strip('/').split('/')
            if len(parts) < 2:
                logger.warning(
                    f"[LICENSE_CHECK] Invalid GitHub URL format: "
                    f"{github_url}"
                )
                return (
                    {
                        'errorMessage':
                            'github_url must point to a valid repository '
                            '(owner/repo)'
                    },
                    400
                )
            ...
            if response.status_code == 404:
                logger.warning(
                    f"[LICENSE_CHECK] GitHub repository not found: "
                    f"{github_url}"
                )
                return (
                    {
                        'errorMessage':
                            f'GitHub repository not found: {github_url}'
                    },
                    404
                )
            elif response.status_code != 200:
                logger.error(
                    f"[LICENSE_CHECK] GitHub API error: "
                    f"{response.status_code}"
                )
                return (
                    {
                        'errorMessage':
                            'External license information could not be '
                            'retrieved'
                    },
                    502
                )
            ...
            logger.info(
                f"[LICENSE_CHECK] GitHub repository exists: "
                f"{owner}/{repo}"
            )
            ...
            logger.error(
                f"[LICENSE_CHECK] Failed to connect to GitHub: {str(e)}"
            )
            return (
                {
                    'errorMessage':
        ...
        if not artifact_license:
            logger.warning(
                "[LICENSE_CHECK] Artifact has no license information"
            )
        ...
        if not github_license:
            logger.warning(
                "[LICENSE_CHECK] GitHub repo has no license information"
            )
        ...
        logger.info(
            ...
            f"(artifact: {artifact_license}, "
            f"github: {github_license})"
        )
