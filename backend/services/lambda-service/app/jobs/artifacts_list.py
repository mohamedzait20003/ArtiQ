...
def lambda_handler(event, context):
    ...
    try:
        ...
        print(
            f"Processing artifacts list request with "
            f"{len(artifact_queries)} queries, "
            f"offset: {offset}"
        )
        ...
        for query in artifact_queries:
            ...
            # Convert artifacts to the expected response format
        ...
        print(
            f"Found {len(unique_results)} unique artifacts after processing "
            f"{len(artifact_queries)} queries"
        )
